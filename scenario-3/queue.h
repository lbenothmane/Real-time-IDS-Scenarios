...
void write_enqueue_data(struct Queue* queue){
  FILE *fptr;
  char path[100];
  snprintf(path, sizeof(char) * 100, "/home/jk/Documents/Fall2021/Research_699/new_code/two_theads_single_process/enqueue/enqueue_%d.txt", queue->rear);
  fptr = fopen(path, "w");
  if (!fptr){
...
void write_dequeue_data(struct Queue* queue, struct Data item){
  FILE *fptr;
  char path[100];
  snprintf(path, sizeof(char) * 100, "/home/jk/Documents/Fall2021/Research_699/new_code/two_theads_single_process/dequeue/dequeue_%d.txt", queue->front);
  fptr = fopen(path, "w");
  if (!fptr){
      error(EXIT_FAILURE, errno, "Failed to open file for writing");
   }
